...
{
	return this->sending((void*)message.c_str(), strlen(message.c_str()) + 1);
}
...
int SocketTCP::receive(std::string& message)
{
	char char_pointer = '\0';
	...
	do {	
		this->receive((void*)&char_pointer, sizeof(char));
		message.push_back(char_pointer);
